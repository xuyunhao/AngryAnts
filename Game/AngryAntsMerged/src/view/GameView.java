...
//	private AnchorPane mediaPlaceholder;	// Displays loading indicator while video is downloaded
	...
	private Button prev;
	private Button next;
	private Button sound;             // Toggles sound
	...
	private String imageView;         // Determines which sound image to use (Sound or Nosound)
	private Profile currProfile;
	
	// ADDED
//	private HBox infoH;
//	private Text gameOps;
	private Button show;              // Shows path
	private Button hide;              // Hides path
	// END: ADDED
...
//	private ProgressIndicator vLoadProgress;
	...
	private int numPrev = 0;                // counts how many frames back the user has gone using previous
	...
	private boolean showPath = false;       // hide path by default
	private boolean isDone = false;         // true only when player has reached end of video; prevents adding extra clicks

	private MediaPlayer APPLAUSE = new MediaPlayer(new Media(getClass().getResource("/sound/Ooooh").toString()));   
	private MediaPlayer CLICK = new MediaPlayer(new Media(GameView.class.getResource("/sound/Quack").toString())); 
	private MediaPlayer DIALOG = new MediaPlayer(new Media(GameView.class.getResource("/sound/tada.wav").toString()));  
	private MediaPlayer MENU = new MediaPlayer(new Media(GameView.class.getResource("/sound/Menu.m4a").toString())); 
	private MediaPlayer PREVNEXT = new MediaPlayer(new Media(GameView.class.getResource("/sound/SoundFX.m4a").toString()));
	private MediaPlayer SHOWHIDE = new MediaPlayer(new Media(GameView.class.getResource("/sound/Wood Bonk").toString())); 
	
	private final ImageView im1 = new ImageView(new Image(getClass().getResource("/img/sound.png").toString(), 36, 36, false, true));
	private final ImageView im2 = new ImageView(new Image(getClass().getResource("/img/Nosound.png").toString(), 36, 36, false, true));
			...
			public void run() {
				...
				switch (antIndex)
				{
					...
					case 44 :
						...
						break;
					default :
						System.out.println("antIndex invalid");
				}
...
//						System.out.println("Drag done:  (" + x2 + ", " + y2 + ")");
						...
						if (currFrameNum == maxFrames) {		// last frame
							...
							Click thisClick = new Click(currFrameNum*50 + 1, x1, y1, degree, Behavior.NONE);       
							//currPath.addClick(new Click(currFrameNum*50 + 1, x1, y1, degree, Behavior.NONE));
							currPath.addClick(thisClick);     
							...
							nextFrame();
							// Add click
							Click thisClick = new Click((currFrameNum-1)*50 + 1, x1, y1, degree, Behavior.NONE);     
							...
							if (showPath && currFrameNum > 2)
							{
								...
								// Draw circle each time around last drag
								double x, y;
								x = endX;
								y = endY;
								circle = new Circle(x + hOffset, y, 25, Color.web("white", 0.05));
...
//		mediaView.setOnMouseReleased(dragReleasedHandler);
		...
		clickHandler = new EventHandler<MouseEvent>() {
			@Override
			public void handle(MouseEvent event) {
//				System.out.println("PLAYER STATUS = " + player.getStatus());  // For debugging
//				System.out.println("(Start, stop) = (" + player.getStartTime() + ", " + player.getStopTime() + ")");  // For debugging
				if (isPaused && !isBuffering && !isInstantReplay) {
					if (event.getButton() == MouseButton.PRIMARY) {		// left click
						// Clicking only works if paused and we have reached the maxFrames
						if (!isAdvanced && !isDone) {
							if (currFrameNum == maxFrames) {		// last frame
								isDone = true;
								CLICK.stop();
								CLICK.play();  // ADDED
	...
	//							submitDialog.show();
								...
								if (showPath && currFrameNum > 2)
								{
									...
									// Draw circle each time around last click
									double x, y;
									x = endX;
									y = endY;
									circle = new Circle(x + hOffset, y, 25, Color.web("white", 0.05));
...
//				System.out.println("End of click handler");  // For debugging
			}
		...
		vidSelect.setFont(Font.font("Chalkduster", 13));  
		vidSelect.setStyle("-fx-base: maroon");  
		vidSelect.setEffect(new InnerShadow());   
		...
		Image tempImg = new Image(getClass().getResource("/img/arrow_left.png").toString(), 50, 36, false, true);
		prev = new Button();
...
//		prev.setPrefSize(45, 35);
		prev.setStyle("-fx-base: orange"); 
		prev.setEffect(new InnerShadow());  
...
//		frameNum.setPrefSize(80, 35);
		...
		frameNum.setEffect(new InnerShadow());  
		frameNum.setStyle("-fx-background-color: darksalmon");  
		
		// Seek next frame button 
		tempImg = new Image(getClass().getResource("/img/arrow_right.png").toString(), 50, 36, false, true);
		next = new Button();
...
//					return "1/2x";
		...
		slider.setStyle("-fx-base: black; -fx-color: darkorange");      
//		slider.getChildren().addAll(speedSlider);
		...
		nav = new HBox();
		nav.getChildren().addAll(vidSelect, sound, prev, frameNum, progress, next, slider);
		nav.setMargin(vidSelect, new Insets(10, 0, 10, 5));
		nav.setMargin(sound, new Insets(8, 0, 0, 70));
		nav.setMargin(prev, new Insets(8, 0, 0, 20));
		nav.setMargin(frameNum, new Insets(10, 0, 0, 10));
		nav.setMargin(progress, new Insets(10, 0, 0, 10));  
		nav.setMargin(next, new Insets(8, 0, 0, 5));
		nav.setMargin(slider, new Insets(10, 0, 0, 35));
		
		// ADDED: Set up show path button
		tempImg = new Image(getClass().getResource("/img/show_path.png").toString(), 120, 40, false, true);
		show = new Button();
		...
		tempImg = new Image(getClass().getResource("/img/hide_path.png").toString(), 120, 40, false, true);
		hide = new Button();
...
//		mediaView.setCursor(Cursor.CROSSHAIR);	// crosshair is transparent for some reason..
	...
	private void initSound() {
		...
		sound.setOnAction(new EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent arg0) {
				controller.toggleSound();  // Update the sound button on IntroScreen too
			}	
		});
	...
	/*
	 * Updates sound button when changed on IntroScreen
	 */
	...
	private void changeVolume(double newVol)
	{
			APPLAUSE.setVolume(newVol);
			CLICK.setVolume(newVol);
			DIALOG.setVolume(newVol);
			MENU.setVolume(newVol);
			PREVNEXT.setVolume(newVol);
			SHOWHIDE.setVolume(newVol);
	}
...
//		mediaPlaceholder.resizeRelocate(0, 0, 960, 540);
		...
		ptDisplay.resizeRelocate(0, 540 + navHeight, 960, navHeight + 5); 
	}
...
//			System.out.println("ERROR: " + status.toString());  // For debugging
			return;
		}
		if (status == Status.PAUSED || status == Status.STOPPED || status == Status.READY) {
//			System.out.println("OK: " + status.toString());  // For debugging
			player.play();
		}
//		System.out.println("OTHER STATUS: " + status.toString());  // For debugging
	}
...
//		prompt.setPrefHeight(30);
	...
	public void nextFrame() {	
		...
		play();
//		System.out.println("Got here");  // For debugging
...
//					SeparatorMenuItemBuilder.create()
	...
	private void showLines(int endFrame)
	{
		...
		if ((numPrev == 0 && endFrame == 2) || (numPrev > 0 && currFrameNum == 1 && currPath.size() > 0))
		{
