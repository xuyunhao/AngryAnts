package edu.arizona {
	...
	public class GamePage extends Sprite {
		...
		private var sWidth:uint, sHeight:uint;
		...
		private function initYouTube(w:uint, h:uint):void {
			...
			yt.video.addEventListener("isReady", function onReady(evt:Event):void {
			...
			yt.video.addEventListener(Event.COMPLETE, onComplete);
			yt.video.addEventListener("replayReady", function(evt:Event):void {
				yt.startReplay();
				overlay.startReplayAnimation();
			});
		...
		private function onComplete(evt:Event):void {
			bar.finish(overlay.getCircle(), sWidth, sHeight);
			
			yt.video.removeEventListener(Event.COMPLETE, onComplete);
			yt.video.addEventListener(Event.COMPLETE, onReplayComplete);
			overlay.prepareReplayAnimation();
			yt.prepareReplay();
		}
			
		private function onReplayComplete(evt:Event):void {
			yt.video.removeEventListener(Event.COMPLETE, onReplayComplete);
			yt.video.addEventListener(Event.COMPLETE, onComplete);
			yt.endReplay();
		}
		...
		private function onMouseClick(evt:MouseEvent = null):void {
			...
			var menuItems:Rectangle = new Rectangle(bar.showhide.x, bar.showhide.y, 
				bar.showhide.width * 4, bar.showhide.width);
			trace("Contains" + menuItems.containsPoint(new Point(mouseX, mouseY)));
			if(!(bar.showMenu && menuItems.containsPoint(new Point(mouseX, mouseY)))){
		...
		private function removeReplayLine():void {
			overlay.removeReplayLine();
			yt.endReplay();
			yt.video.removeEventListener(Event.COMPLETE, onReplayComplete);
			yt.video.addEventListener(Event.COMPLETE, onComplete);
		}
