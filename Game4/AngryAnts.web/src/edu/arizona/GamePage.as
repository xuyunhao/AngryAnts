package edu.arizona {
	...
	public class GamePage extends Sprite {
		...
		private var startDataState:Number;
		...
		private function initOverlay(w:uint, h:uint):void {
			sh = h;
			funFact = new FunFactManager();
			overlay = new TouchOverlay(w, h - HudBar.HUD_HEIGHT, funFact);
...
//			trace("Contains" + menuItems.containsPoint(new Point(mouseX, mouseY)));
		...
		private function initHud(w:uint, h:uint):void {
			bar = new HudBar(w, h, yt, overlay, funFact);
		...
		private function receiveStart():void {
			...
			function varsLoaded(event:Event):void {
				...
				startDataState = loader.data.dataState;
				...
				trace("url3: " + url3);
				trace(loader.data);
				overlay.prepareToTrack(knownPoints[0].x, knownPoints[0].y, startDataState);
				funFact.prepare();
				funFact.hide = false;
				
				
				// update video URLs
				yt.setVideo1xString(url1);
				yt.setVideo2xString(url2);
				yt.setVideo4xString(url3);
				
				// force the video to load correctly
				yt.setSpeed2x();
				yt.setSpeed1x();
			}
		...
		/*
		private function receiveStartCount():void {
			var loader:URLLoader = new URLLoader();
			loader.dataFormat = URLLoaderDataFormat.VARIABLES;
			loader.addEventListener(Event.COMPLETE, varsLoaded);
			loader.addEventListener(IOErrorEvent.IO_ERROR, onIOError);
...
*/
		public function startCount():void {
			...
			var request:URLRequest = new URLRequest("http://angryants.cs.arizona.edu/data/startCount.php");
			loader.load(request);
		...
		private function restart(evt:MouseEvent = null):void {
			...
			overlay.setFinishedState(false);			
			bar.quitHelper();
			if(knownPoints){
				overlay.reload(knownPoints[0], knownPoints[0], startDataState); 
			}
