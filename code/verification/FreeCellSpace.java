...
//            	int ran = random.nextInt(3)+2;
            	...
            	for (Double percent: percents) {
            		...
            		Point random_P = computePoint(P, x3);
            		Point random_Q = computePoint(Q, y3);
   ...
   public PolyCurve getMedian() {
	...
	xs = new ArrayList<Double>();
       if (solution.size() > 0)
       {
          ...
          x1 = ((Double)solution.elementAt(solution.size() -2)).doubleValue();
          y1 = ((Double)solution.elementAt(solution.size() -1)).doubleValue();
	...
	double px = x1;
	xs.add(px);
          for (int i = solution.size() / 2 - 2; i > 0; i--)
          {
          	x2 = ((Double)solution.elementAt(i * 2)).doubleValue();
          	y2 = ((Double)solution.elementAt(i * 2 + 1)).doubleValue();
...
//          	result += "\n";
   ...
   private Point computePoint(PolyCurve PC, double x) {
		double x_p1 = ((Point)(PC.getPoint((int)x))).getX();
		double y_p1 = ((Point)(PC.getPoint((int)x))).getY();
		int n = (int)x+1;
		if (n>=PC.getSize()) n=n-1;
		double x_p2 = ((Point)(PC.getPoint(n))).getX();
		double y_p2 = ((Point)(PC.getPoint(n))).getY();
		
		double x_sp = x_p1 + (x_p2 -x_p1) * (x - (int)x);
		double y_sp = y_p1 + (y_p2 -y_p1) * (x - (int)x);
		Point p = new Point();
